...
var _ = Describe("kumactl get dataplanes", func() {
	...
	Describe("GetDataplanesCmd", func() {
		...
		Describe("kumactl get dataplanes --tag", func() {
		...
		Describe("kumactl get dataplanes --gateway", func() {
			It("gateway should be passed to the client", func() {
				// given
				rootCmd.SetArgs([]string{
					"--config-file", filepath.Join("..", "testdata", "sample-kumactl.config.yaml"),
					"get", "dataplanes", "--gateway"})
		...
		DescribeTable("kumactl get dataplanes --compact -o table|json|yaml",
			func(given testCase) {
				// given
				rootCmd.SetArgs(append([]string{
					"--config-file", filepath.Join("..", "testdata", "sample-kumactl.config.yaml"),
					"get", "dataplanes", "--compact"}, given.outputFormat))
			...
			Entry("should support Table output by default", testCase{
				outputFormat: "",
				goldenFile:   "get-dataplanes-compact.golden.txt",
				matcher: func(expected interface{}) gomega_types.GomegaMatcher {
					return WithTransform(strings.TrimSpace, Equal(strings.TrimSpace(string(expected.([]byte)))))
				},
			}),
			...
			Entry("should support JSON output", testCase{
				outputFormat: "-ojson",
				goldenFile:   "get-dataplanes-compact.golden.json",
				matcher:      MatchJSON,
			}),
			Entry("should support YAML output", testCase{
				outputFormat: "-oyaml",
				goldenFile:   "get-dataplanes-compact.golden.yaml",
				matcher:      MatchYAML,
			}),
		)
	})
